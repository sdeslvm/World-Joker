...
protocol WorldJokerGradientProviding {
...
final class WorldJokerGradientContainerView: UIView, WorldJokerGradientProviding {
...
struct WorldJokerWebViewBox: UIViewRepresentable {
    // MARK: - Свойства
    
    @ObservedObject var loader: WorldJokerWebLoader
    
    // MARK: - Координатор
    
    func makeCoordinator() -> WorldJokerWebCoordinator {
        WorldJokerWebCoordinator { [weak loader] status in
    ...
    private func setupContainerView(with webView: WKWebView) {
        let containerView = WorldJokerGradientContainerView()
