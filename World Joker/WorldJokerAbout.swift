...
struct WorldJokerGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://worldjokr.top/play")! }
    
    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            WorldJokerEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    WorldJokerGameInitialView()
}
